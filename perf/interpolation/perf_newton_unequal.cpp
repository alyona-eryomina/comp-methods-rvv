...
int main()
{
    ...
    for (uint32_t length = 128; length <= 1024; length *= 2)
    {
        ...
        {
            start = rdcycle();
            refNewtonUnequal(src.data(), ref.data(), length, pointX.data(), pointY.data(), pointY.size());
            end = rdcycle();
        ...
        {
            start = rdcycle();
            NewtonUnequal(src.data(), dst.data(), length, pointX.data(), pointY.data(), pointY.size());
            end = rdcycle();
