...
extern inline void spline_cub_rvv(float* src, float* dst, uint32_t length, float* pointX, float* pointY, uint32_t pointLength, SplineCubBuf* buf)
{
    ...
    for (;(len > 1);)
    {
        ...
        vse32_v_f32m8(buf->va + offset, vA, l);
        vse32_v_f32m8(buf->vb + offset, vB, l);
        vse32_v_f32m8(buf->vc + offset, vC, l);
        vse32_v_f32m8(buf->F + offset, vF, l);
    ...
    for (;(len > 0);)
    {
        ...
        vse32_v_f32m8(buf->c + offset, vC, l);
    ...
    for (;(len > 0);)
    {
        ...
        vPointX = vloxei32_v_f32m8(pointX, vNum, l);
        vPointY = vloxei32_v_f32m8(pointY, vNum, l);
        vX = vfsub_vv_f32m8(vSrc, vPointX, l);

        vA = vloxei32_v_f32m8(buf->a, vNum, l);
        vB = vloxei32_v_f32m8(buf->b, vNum, l);
        vC = vloxei32_v_f32m8(buf->c, vNum, l);
