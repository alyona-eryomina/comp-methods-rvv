...
extern inline uint32_t conjugate_gradient_rvv(uint32_t size, double* A, double* b, double* x, double eps, SLAMGradBuf* buf) {
    ...
    for (;(len > 0);) {
        ...
        vX = vfmv_v_f_f64m2(0.0, l);
        vse64_v_f64m2(x + offset, vX, l);

        vR = vle64_v_f64m2(b + offset, l);
        vse64_v_f64m2(buf->r + offset, vR, l);
        vse64_v_f64m2(buf->z + offset, vR, l);
        
        vR = vfmul_vv_f64m2(vR, vR, l);
        vSpr0 = vfredosum_vs_f64m2_f64m1(vSpr0, vR, vSpr0, l);
    ...
    do {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (;(len > 0);) {
                ...
                vR = vle64_v_f64m2(A + i * size + offset, l);
                vZ = vle64_v_f64m2(buf->z + offset, l);

                vR = vfmul_vv_f64m2(vR, vZ, l);
                tmp = vfredosum_vs_f64m2_f64m1(tmp, vR, tmp, l);
        ...
        for (;(len > 0);) {
            ...
            vS = vle64_v_f64m2(buf->s + offset, l);
            vZ = vle64_v_f64m2(buf->z + offset, l);

            vS = vfmul_vv_f64m2(vS, vZ, l);
            vSpz = vfredosum_vs_f64m2_f64m1(vSpz, vS, vSpz, l);
        ...
        for (;(len > 0);) {
            ...
            vZ = vle64_v_f64m2(buf->z + offset, l);
            vX = vfmacc_vf_f64m2(vX, alpha, vZ, l);
            vse64_v_f64m2(x + offset, vX, l);
            
            vR = vle64_v_f64m2(buf->r + offset, l);
            vS = vle64_v_f64m2(buf->s + offset, l);
            vR = vfnmsac_vf_f64m2(vR, alpha, vS, l);
            vse64_v_f64m2(buf->r + offset, vR, l);

            vR = vfmul_vv_f64m2(vR, vR, l);
            vSpr1 = vfredosum_vs_f64m2_f64m1(vSpr1, vR, vSpr1, l);
        ...
        for (;(len > 0);) {
            ...
            vZ = vle64_v_f64m2(buf->z + offset, l);
            vR = vle64_v_f64m2(buf->r + offset, l);
            vZ = vfmadd_vf_f64m2(vZ, beta, vR, l);
            vse64_v_f64m2(buf->z + offset, vZ, l);
