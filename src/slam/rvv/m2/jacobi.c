...
extern inline uint32_t jacobi_rvv(uint32_t size, double* A, double* b, double* x, double eps, SLAMJacobiBuf* buf) {
    ...
    vfloat64m2_t vTmpX, vA, vX, vAX, vNorm;
    ...
    do {
        ...
        for (;(len > 0);) {
            size_t l = vsetvl_e64m2(len);
            vTmpX = vle64_v_f64m2(b + offset, l);
            vAX = vle64_v_f64m2(buf->tmp_x + offset, l);
    
            vInd = vle64_v_u64m2(buf->index + offset, l);
            vInd = vmul_vx_u64m2(vInd, sizeof(double), l);
            vA = vloxei64_v_f64m2(A, vInd, l);
            vX = vle64_v_f64m2(x + offset, l);
            
            vAX = vfmsub_vv_f64m2(vA, vX, vAX, l);
            vTmpX = vfadd_vv_f64m2(vTmpX, vAX, l);
            vTmpX = vfdiv_vv_f64m2(vTmpX, vA, l);
            vse64_v_f64m2(x + offset, vTmpX, l);

            vNorm = vfsub_vv_f64m2(vX, vTmpX, l);
            vNorm = vfabs_v_f64m2(vNorm, l);
            norm1 = vfredmax_vs_f64m2_f64m1(norm1, vNorm, norm1, l);
