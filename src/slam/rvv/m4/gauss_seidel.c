...
static int converge(double* x, double* prev_x, int size, double eps) {
    ...
    for (;(len > 0);) {
        ...
        vP = vle64_v_f64m4(prev_x + offset, l);
        vX = vfsub_vv_f64m4(vX, vP, l);
        vX = vfmul_vv_f64m4(vX, vX, l);
        vNorm = vfredosum_vs_f64m4_f64m1(vNorm, vX, vNorm, l);
...
extern inline uint32_t gauss_seidel_rvv(uint32_t size, double* A, double* b, double* x, double eps, SLAMGaussBuf* buf) {
    ...
    do {
        ...
        for (;(len > 0);) {
            size_t l = vsetvl_e64m4(len);
            vX = vle64_v_f64m4(x + offset, l);
            vse64_v_f64m4(buf->tmp_x + offset, vX, l);
